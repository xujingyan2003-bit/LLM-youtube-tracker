    ...
    if "youtube.com/@" in rss_url or "/channel/" in rss_url or "/c/" in rss_url:
        ...
        for match in re.finditer(r"/watch\?v=([A-Za-z0-9_-]{11})", response.text):
            ...
            try:
                ...
                detail_resp = requests.get(video_url, headers={"User-Agent": "Mozilla/5.0"}, timeout=20)
                detail_resp.raise_for_status()
                # 1. 先尝试 JSON-LD 结构化数据
                match_date = re.search(r'"datePublished"\s*:\s*"([^"]+)"', detail_resp.text)
                if match_date:
                    published = match_date.group(1)
                # 2. 如果没有，尝试抓取“X天前/周前/月前”
                if not published:
                    # 匹配 <span ...>3周前</span> 或 <span ...>2天前</span>
                    match_relative = re.search(r'<span[^>]*>([0-9一二三四五六七八九十]+[ ]?(秒|分钟|小时|天|周|月|年)前)</span>', detail_resp.text)
                    if match_relative:
                        published = match_relative.group(1)
            except Exception:
                published = None
            videos.append(
                {
